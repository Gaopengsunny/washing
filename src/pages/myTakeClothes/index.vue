		...
		<view class="container">
			...
			<scroll-view class="orderList" scroll-y="true" :lower-threshold="50" v-if="btnStatus !== 30" @scrolltolower="orderDetailList">
				<view class="box" v-for="(item,index) in orderlist" :key="index">
					...
					<taken-clothes v-if="btnStatus === 7" :orderInfo="item" @onClickOrder="onClickOrder" @onSucess="onSucess" @sealed="sealed"
					 @goStore="goStore"></taken-clothes>
					<submit-store v-if="btnStatus === 17" :orderInfo="item" @onClickOrder="onClickOrder"></submit-store>
					<fail-taken v-if="btnStatus === 1" :orderInfo="item" @onClickOrder="onClickOrder"></fail-taken>
				</view>
...
<script>
	import UntakenClothes from '@/pages/index/components/untakenClothes'
	import TakenClothes from '@/pages/index/components/takenClothes'
	import SubmitStore from '@/pages/index/components/submitStore'
	import oldsubmitStore from '@/pages/index/components/oldsubmitStore.vue'
	import failTaken from '@/pages/index/components/failTaken.vue'
	...
	export default {
		...
		methods: {
			...
			updataOrderStatus(this.selParam).then(res => {
				...
				this.orderlist.splice(this.selParam.index, 1)
				this.closeel()
			})
