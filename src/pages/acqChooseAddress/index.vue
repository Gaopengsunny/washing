    ...
    <view class="view-body">
        <view class="v-body100">
            ...
            <button class="v-col3 icon iconfont icon-banka" type="primary" plain @click="goCard()">
                <!-- <cmd-icon type="credit-card" color="#ffffff" size="14"></cmd-icon> -->
                办卡
            </button>
            ...
            <view class="address-box">
                <view class="address-title">
                    <view class="address-title-left">
                        ...
                        <view class="title">选择<text class="type">取件</text>地址</view>
                    </view>
...
export default {
    ...
    methods: {
        getUserAddress(user) {
            acqGetAddress({ "userId":user.userId }).then(res => {
		...
		next(){
			...
			let  addressId = this.addressList[this.selIndex].addressId
			uni.navigateTo({
			  url: `/pages/acqdistribution/index?user=${this.user.userId}&doorInAddressId=${addressId}`,
			});
		},
		again(){
			uni.switchTab({
