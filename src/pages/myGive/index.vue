			...
			<scroll-view scroll-y="true" class="orderList">
				<view class=""  v-for="(clothes, index) in orderlist" :key="index">
					<un-send-card v-if="btnStatus === 4" @onClickOrder="onClickOrder" :key="index" :index="index" :clothes="clothes"
						 @onFail="onFail" @onSucess="onSucess" @onSend="onSend"></un-send-card>
...
<script>
	import UnSendCard from '@/pages/give/components/unsendCard'
	import SendCard from '@/pages/give/components/sendCard'
	import failSend from '@/pages/give/components/failSend'
	import {
		orderDetailList,
		updataOrderStatus,
		orderAbnormal,
