...
<script>
	export default {
		...
		props: {
			...
			cityData: { //数据 如：[{id: '', name: '', children: [{id: '', name: ''}]}]
				type: Array,
				default (){
					return [
	...
	　　},
		...
		methods:{
			...
			_chooseItem(e){
				...
				if(
					(!_this._isDefine(_this.layer) && _this._isDefine(_this._deepQuery(_this.cityData, id).children)) 
					|| 
					(_this.navCurrentIndex < (Number(_this.layer) - 1) && _this._isDefine(_this._deepQuery(_this.cityData, id).children))
				)
				{ //有下级数据
					_this._deepQuery(_this.cityData, id).children.forEach(function(item, index){
